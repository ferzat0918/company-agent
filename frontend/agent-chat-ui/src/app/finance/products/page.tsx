...
export default function FinanceProductsPage() {
  ...
  const remove = async (r: Product) => {
    if (!confirm(`确认删除成品 ${r.code} ${r.name}？流水将保留。`)) return;
    const { error } = await supabase.from("fin_products").delete().eq("id", r.id);
