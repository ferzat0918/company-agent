...
function renderInterruptStateItem(value: any): React.ReactNode {
  if (isComplexValue(value)) {
    return (
      <code className="rounded-[2px] bg-[var(--umx-bg-1)] px-2 py-1 font-mono text-sm text-[var(--umx-silver)]">
    ...
    return (
      <a
        ...
        className="break-all text-[var(--umx-acid)] underline hover:brightness-110"
      >
        {value}
      </a>
    );
...
export function GenericInterruptView({
  ...
  return (
    <div className="overflow-hidden rounded-[2px] border border-[var(--umx-line)]">
      <div className="border-b border-[var(--umx-line)] bg-[var(--umx-bg-1)] px-4 py-2">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <h3 className="font-mono text-[11px] uppercase tracking-[0.18em] text-[var(--umx-violet)]">⚠ HUMAN INTERRUPT</h3>
        </div>
      </div>
      <motion.div
        className="min-w-full bg-[var(--umx-bg-2)]"
        initial={false}
        animate={{ height: "auto" }}
      ...
      >
              ...
              <table className="min-w-full divide-y divide-[var(--umx-line)]">
                <tbody className="divide-y divide-[var(--umx-line)]">
                  {displayEntries.map((item, argIdx) => {
                    ...
                    return (
                      <tr key={argIdx}>
                        <td className="px-4 py-2 font-mono text-[11px] font-medium whitespace-nowrap text-[var(--umx-white)]">
                          {key}
                        </td>
                        <td className="px-4 py-2 text-sm text-[var(--umx-text-dim)]">
        ...
        {(shouldTruncate ||
          ...
          <motion.button
            ...
            className="flex w-full cursor-pointer items-center justify-center border-t-[1px] border-[var(--umx-line)] py-2 text-[var(--umx-text-dim)] transition-all duration-200 ease-in-out hover:bg-[var(--umx-bg-3)] hover:text-[var(--umx-silver)]"
            initial={{ scale: 1 }}
