    ...
    <div className="flex w-full flex-col gap-1">
      {messages.map((msg, idx) => {
        ...
        return (
          ...
          >
            <p className="font-mono text-[11px] uppercase tracking-[0.18em] text-[var(--umx-silver)]">{messageTypeLabel}:</p>
...
function StateViewRecursive(props: StateViewRecursiveProps) {
  ...
  if (date) {
    return <p className="font-light text-[var(--umx-text-dim)]">{date}</p>;
  }
  ...
  if (props.value == null) {
    return <p className="font-light whitespace-pre-wrap text-[var(--umx-text-dim)]">null</p>;
  }
  ...
  if (Array.isArray(props.value)) {
    ...
    return (
      <div className="flex w-full flex-row items-start justify-start gap-1">
        <span className="font-normal text-[var(--umx-white)]">[</span>
  ...
  if (typeof props.value === "object") {
    if (Object.keys(props.value).length === 0) {
      return <p className="font-light text-[var(--umx-text-dim)]">{"{}"}</p>;
    }
    return (
      <div className="relative ml-6 flex w-full flex-col items-start justify-start gap-1">
        {/* Vertical line */}
        <div className="absolute top-0 left-[-24px] h-full w-[1px] bg-[var(--umx-line)]" />
        ...
        {Object.entries(props.value).map(([key, value], idx) => (
          ...
          >
            ...
            <div className="absolute top-[10px] left-[-20px] h-[1px] w-[18px] bg-[var(--umx-line)]" />
            <StateViewObject
  ...
  return (
    <span
      ...
      className={cn(
        ...
        "bg-[var(--umx-bg-2)] text-[var(--umx-text-dim)] hover:bg-[var(--umx-bg-3)] hover:text-[var(--umx-white)]",
    ...
    <div className="relative flex flex-row items-start justify-start gap-2 text-sm">
      ...
      >
        <div
          ...
          className="flex h-5 w-5 cursor-pointer items-center justify-center rounded-[2px] text-[var(--umx-text-dim)] transition-colors ease-in-out hover:bg-[var(--umx-bg-2)] hover:text-[var(--umx-white)]"
        >
          <ChevronRight className="h-4 w-4" />
        </div>
      </motion.div>
      <div className="flex w-full flex-col items-start justify-start gap-1">
        <p className="font-normal text-[var(--umx-white)]">
  ...
  return (
    <div
      className={cn(
        ...
        view === "state" &&
          "border-t-[1px] border-[var(--umx-line)] lg:border-t-[0px] lg:border-l-[1px]",
      )}
    >
      {view === "description" && (
