...
_SANDBOX_SUFFIX = (
    ...
    "7. 请注意：你自身**没有**直接在宿主机写入或编辑文件的能力（直接调用 write_file / edit_file 等底层工具会被安全权限拦截），因此所有文件生成、编辑、转换和复杂数据解析都**必须**通过在 `execute_python_in_sandbox` 中编写 Python 代码来完成。"
...
_wechat_channel_prompt = _load_prompt(
    ...
    fallback="【渠道：微信】请使用纯文本格式回复，禁止 Markdown，300字以内。",
)
_web_channel_prompt = _load_prompt(
    "shared/channel_web.md",
    fallback="【渠道：Web】可以使用 Markdown 排版，提供下载链接。",
)
...
agent = create_deep_agent(
    ...
    middleware=[
        _memory_middleware,
        WeChatChannelMiddleware(
            wechat_channel_prompt=_wechat_channel_prompt,
            web_channel_prompt=_web_channel_prompt,
        ),
    ],
)
