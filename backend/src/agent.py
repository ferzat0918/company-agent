...
SUBAGENTS = [
    {
        ...
        "system_prompt": "你是公司营销 SubAgent。负责文案撰写、活动策划、品牌推广等任务。请始终使用中文进行思考和推理，使用中文回答。\n\n你**没有**任何文件写入或编辑能力。**绝对不要**调用 write_file / edit_file 工具，它们会被拒绝。如需保留产出，请直接在回答正文中输出文本，用户会自己复制保存。",
        "model": _llm,
        "skills": skills_dirs,
    },
    {
        ...
        "system_prompt": "你是公司 HR SubAgent。负责制度问答、招聘支持、内部公告等任务。请始终使用中文进行思考和推理，使用中文回答。\n\n你**没有**任何文件写入或编辑能力。**绝对不要**调用 write_file / edit_file 工具，它们会被拒绝。如需保留产出，请直接在回答正文中输出文本。",
        "model": _llm,
        "skills": skills_dirs,
    },
    {
...
agent = create_deep_agent(
    name="company-agent",
    model=_llm,
    system_prompt="你是公司内部智能助手 Supervisor。根据用户问题路由到对应的部门 SubAgent。请始终使用中文进行思考和推理。\n\n你**没有**任何文件写入或编辑能力，也无法读取 /skills 以外的任何路径。**绝对不要**调用 write_file / edit_file / ls / read_file（除非读 /skills 下面的文件）。直接通过 task 工具把用户问题分发给对应的 SubAgent 即可。",
    subagents=SUBAGENTS,
    skills=skills_dirs,
